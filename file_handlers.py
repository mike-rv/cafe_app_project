...
if is_csv_empty('products.csv'):
    ...
    with open('products.csv') as csv_file:
        products_list = [{k: v for k, v in row.items()} for row in csv.DictReader(csv_file)]
        
if is_csv_empty('couriers.csv'):
    couriers_list = []
else:
    with open('couriers.csv') as csv_file:
        couriers_list = [{k: v for k, v in row.items()} for row in csv.DictReader(csv_file)]

if is_csv_empty('orders.csv'):
    orders_list = []
else:
    with open('orders.csv') as csv_file:
        orders_list = [{k: v for k, v in row.items()} for row in csv.DictReader(csv_file)]
