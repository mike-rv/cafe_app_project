...
def create(list_type, menu_type):
    ...
    temp_dict.update({'id' : ''.join([value[:2] for key, value in temp_dict.items() if len(temp_dict) == 3 and key == 'name']) + ''.join([value for key, value in temp_dict.items() if len(temp_dict) == 3 and key == 'id']) + ''.join([value[-1] for key, value in temp_dict.items() if len(temp_dict) == 3 and key == 'name'])})
...
def update(list_type, menu_type):
    if len(list_type) == 0:
        ...
        if info in [value['id'] for value in list_type]:
            index = next((index for (index, key) in enumerate(list_type) if key["id"] == info), None)
            for key, _ in list_type[int(index)].items():
                if key == 'price':
                    ...
                    if not key == 'id':
                        ...
                        continue
            list_type[index]['id'] = ''.join([value[:2] for key, value in list_type[index].items() if len(list_type[index]) == 3 and key == 'name']) + ''.join([value for key, value in list_type[index].items() if len(list_type[index]) == 3 and key == 'id']) + ''.join([value[-1] for key, value in list_type[index].items() if len(list_type[index]) == 3 and key == 'name'])
        else:
            a.incorrect_input()    
    view_list(list_type, menu_type)
        ...
        if [value['id'] for value in list_type if info in value['id']]:
            ...
            for item in fh.orders_list:
                for key, _ in item.items():
                    if list_type == fh.products_list and key == 'product_id_index' and str(index+1) in item[key]:
                        item.update({'product_id_index' : ','.join([value for value in [*item[key]] if value != ',' and value != str(index+1)])})
                    elif list_type == fh.couriers_list and key == 'courier_id' and str(index+1) in item[key]:
                        item.update({'courier_id' : ','.join([value for value in [*item[key]] if value != ',' and value != str(index+1)])})
            for item in fh.orders_list:
