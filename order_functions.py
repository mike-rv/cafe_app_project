...
def create_order(list_type, menu_type):
    temp_dict = {"customer_name": '', 'customer_address': '', 'customer_phone_number': '', 'product_id_index': '','courier_id': '', 'status': 'preparing'}
    for key, _ in temp_dict.items():
        if not key == 'status':
            if key == 'product_id_index': 
                for item in fh.products_list:
                    print(item)
                product_id_info = [id for id in input("Input product id(s). Leave space for multiple ids\n").split()]
                id_str_list = ''
                ...
                if input_info_str not in [value['id'] for value in fh.couriers_list]:
                    print(f'id {input_info_str} does not exist. Order details cancelled')
                    a.sub_menu(list_type, menu_type)
                else:
    ...
    if len(list_type) == 0:
        ...
        try:
            ...
            if order_number == 0 or order_number < len(list_type):
                ...
                order_number_status = int(input("""To change status, input: 
1 for preparing
2 for dispatched
3 for delivered
4 to return to menu \n"""))
        ...
        for key, _ in fh.orders_list[order_number].items():
            if key == 'status':
                continue
                        ...
                        id_str_list = ''
                        for id in product_id_info:
                            if product_id_info and id not in [value['id'] for value in fh.products_list]:
                                print(f'id {id} does not exist. Order details cancelled')
                                a.sub_menu(list_type, menu_type) 
                        ...
                        for courier in fh.couriers_list:
                            print(courier)
                        input_info_str = input(f"Input {key.replace('_', ' ')}\n")
                        if input_info_str and input_info_str not in [value['id'] for value in fh.couriers_list]:
                            print(f'id {input_info_str} does not exist. Order details cancelled')
                            a.sub_menu(list_type, menu_type)
                        else:
                            if not input_info_str:
